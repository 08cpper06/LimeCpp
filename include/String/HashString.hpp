...
public:
	...
	THashString(const TUtf32String& InStr)
	{
		...
		CalcHashValue(String::ConvertToUtf8(Str.Bytes())->Bytes());
		auto Itr = StringTable.find(MyHashValue);
		if (Itr == StringTable.end())
		{
			StringTable.insert({ MyHashValue, LastIndex++ });
	...
	{
		return MyHashValue == InRhs.MyHashValue;
	}
	bool operator!=(const THashString& InRhs) const
	{
		return MyHashValue != InRhs.MyHashValue;
...
private:
	...
	constexpr void CalcHashValue(TIterator Itr, TIterator End)
	{
		Lime::size_t Length = 0;
		while (Itr != End)
		{
			for (Lime::size_t Index = 0; Index < 4; ++Index)
			{
				MyHashValue = (MyHashValue << 8) ^ RefTable(static_cast<unsigned char>((MyHashValue >> 24) ^ (*Itr).MyData[Index]));
			}
			++Length;
			++Itr;
		}
...
private:
	inline static constexpr Lime::size_t CRC32TableSize = std::numeric_limits<char8_t>::max() + 1;
	inline static constexpr Lime::size_t CRC32Table[THashString::CRC32TableSize] = {
...
std::basic_ostream<char, _Traits>& operator<<(std::basic_ostream<char, _Traits>& _Ostr, const THashString& _Val)
{
